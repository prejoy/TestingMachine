/*******************************************************************************
 * File				:	dev_INA219.c
 * Description		:	STM32F4x library for INA219 sensor which reads
...
uint32_t ina219_calValue,ina219_calValue_2;
// The following multipliers are used to convert raw current and power
// values to mA and mW, taking into account the current config settings
uint32_t ina219_currentDivider_mA,ina219_currentDivider_mA_2;
uint32_t ina219_powerDivider_mW,ina219_powerDivider_mW_2;

/* Internal Functions *********************************************************/
void ina219_gpio_init(void);
void ina219_i2c_init(void);
void ina219_configureRegisters(uint32_t index);
...
* Attention			:	None
*******************************************************************************/
void ina219_init(uint32_t index){
...
//	ina219_i2c_init();
	...
	ina219_configureRegisters(index);
}
...
void ina219_configureRegisters(uint32_t index){
	if(index == 1){
		...
		ina219_powerDivider_mW_2 = 0;
	}


	IIC_Init(index);

	ina219_setCalibration_16V_400mA(index);
}
...
* Attention			:	None
*******************************************************************************/
void ina219_writeRegister(uint8_t reg, uint16_t data,uint32_t index){
/*
...
*******************************************************************************/
void ina219_readRegister(uint8_t reg, uint16_t *data,uint32_t index){
/*
	 Wait while the bus is busy
	while(I2C_GetFlagStatus(INA219_I2C, I2C_FLAG_BUSY));
...
void ina219_setCalibration_32V_2A(void){
	...
	ina219_writeRegister(INA219_REG_CALIBRATION, ina219_calValue,0);
	...
	ina219_writeRegister(INA219_REG_CONFIG, config,0);
}
...
* Attention			:	These calculations assume a 0.1 ohm resistor is present
*******************************************************************************/
void ina219_setCalibration_16V_400mA(uint32_t index){
	...
	// Cal = 8192 (0x2000)
	if(index == 1)
		ina219_calValue = 8192;		//34=12uA
	else if (index == 2)
		ina219_calValue_2 = 8192;		//34=12uA
	...
	if(index == 1){
		...
		ina219_powerDivider_mW_2 = 1;     // Power LSB = 1mW per bit
	}

	// Set Calibration register to 'Cal' calculated above
	ina219_writeRegister(INA219_REG_CALIBRATION, ina219_calValue,index);
...
//					INA219_CONFIG_MODE_SANDBVOLT_CONTINUOUS;
	...
	ina219_writeRegister(INA219_REG_CONFIG, config,index);
}
...
int16_t ina219_getBusVoltage_raw(void){
	uint16_t value;
	ina219_readRegister(INA219_REG_BUSVOLTAGE, &value,0);
...
int16_t ina219_getShuntVoltage_raw(void){
	uint16_t value;
	ina219_readRegister(INA219_REG_SHUNTVOLTAGE, &value,0);
...
//	return (int16_t)value;
//}
int32_t ina219_getCurrent_raw(uint32_t index){
	int16_t value;
	...
	// Now we can safely read the CURRENT register!
	ina219_readRegister(INA219_REG_CURRENT, &value,index);
...
* Attention			:	None
*******************************************************************************/
float ina219_getCurrent_mA(uint32_t index){
//	float valueDec = ina219_getCurrent_raw();
//	valueDec /= ina219_currentDivider_mA;
//	return valueDec;

	float retval=0.000;
	int32_t valueDec=ina219_getCurrent_raw(index);
//	printf("I2C%d raw dat:%d  ",index,valueDec);
	retval = valueDec*0.05;//*0.012
	return  retval;
}
...
void ina219_powerOnReset(void){
	ina219_writeRegister(INA219_REG_CONFIG, INA219_CONFIG_RESET,0);
}
...
void ina219_updateConfigurationRegister(uint16_t reg_bits, INA219Mask_Typedef mask){
	...
	ina219_readRegister(INA219_REG_CONFIG, &reg_value,0);
	...
	ina219_writeRegister(INA219_REG_CONFIG, reg_value,0);
}
