...
static U16 _2GB2312(U8 c0, U8 c1) {return c1|(((U16)c0)<<8);}
...
static int _GetLineDistX(const char GUI_UNI_PTR *s, int Len)
{
	...
	while(((c0=*(const U8*)s) != 0) && Len > 0)
	{
		...
		{
			U8  c1 = *(const U8*)s++;
			Dist += GUI_GetCharDistX(_2GB2312(c0, c1));
		}
...
static void _DispLine(const char GUI_UNI_PTR *s, int Len)
{
	...
	while(--Len >= 0)
	{
		...
		{
			c1 = *(const U8*)s++;
			pProp = _FindChar((const GUI_FONT_PROP_GL GUI_UNI_PTR *)pAFont->p.pProp, _2GB2312(c0, c1));
			if(!pProp) {GLCD_WCMD_END(0); goto EXIT;}
			GLCD_WCMD_PAR(_2GB2312(c0, c1)>>8); GLCD_WCMD_PAR(_2GB2312(c0, c1));
			sizeX += (pAFont->Encode&16) ? pProp->pXsize[_2GB2312(c0, c1)-pProp->First] : pProp->pXsize[0];
		}
		else
		{
